...
async function runRead(args: { query?: string, tags?: string[] }): Promise<ManageMemoryReadResult> {
  ...
  if (hasQuery && hasTags) {
    results = await searchMemories(args.query!.trim(), args.tags)
  }
  else if (hasQuery) {
    results = await searchMemories(args.query!.trim())
  }
  else if (hasTags) {
    results = await searchMemoriesByTags(args.tags!)
  }
  else {
...
export async function executeManageMemory(
  ...
  switch (args.action) {
    case 'list': {
      ...
      const results = await listMemories({ limit: pageSize + 1, offset })
