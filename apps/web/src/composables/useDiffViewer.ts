...
export interface UseDiffViewerProps {
...
export function useDiffViewer(
  props: UseDiffViewerProps,
  emit: {
    annotate: (range: SelectedLineRange) => void
  },
) {
...
`
  ...
  // Include annotations: FileDiff.setLineAnnotations() only updates state; DOM is refreshed in full render().
  watch(
    () => [
      props.patch,
      props.diffStyle,
      props.filePath,
      props.enableAnnotation,
      isDark.value,
      props.annotations,
    ],
    render,
